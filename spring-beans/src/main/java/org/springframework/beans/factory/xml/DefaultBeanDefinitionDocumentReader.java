...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	 ...
	 */
	// 更具Spring DTD对Bean的定义规则解析Bean定义的文档对象
	@Override
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		// 具体的解析过程由BeanDefinitionParserDelegate实现，BeanDefinitionParserDelegate中定义了Spring Bean定义XML文件中的各种元素
		...
		// 解析Bean定义之前，进行自定义解析，增强解析过程的可拓展性
		preProcessXml(root);
		// 从文档的根元素开始进行Bean定义的文档对象解析
		parseBeanDefinitions(root, this.delegate);
		// 在解析Bean定义之后，进行自定义解析，增加解析过程的可拓展性
		postProcessXml(root);
	...
	private void parseDefaultElement(Element ele, BeanDefinitionParserDelegate delegate) {
		...
		// 如果节点既不是导入元素，也不是别名与纳素，而是普通的<bean>元素，则按照Spring的Bean规则解析元素
	...
	protected void processBeanDefinition(Element ele, BeanDefinitionParserDelegate delegate) {
		...
		if (bdHolder != null) {
			// BeanDefinitionHolder是对BeanDefinition的封装，即Bean定义的封装类对文档对象中bean元素的解析由BeanDefinitionParserDelegate实现
			...
			// 在完成向Spring IOC容器注册解析得到Bean之后，发送注册事件
